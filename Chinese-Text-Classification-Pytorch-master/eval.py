...
class NewsClassifier:
    def __init__(self):
        ...
        self.model_name = 'FastText'  # TextCNN, TextRNN, FastText, TextRCNN, TextRNN_Att, DPCNN, Transformer

        self.labels = ["finance", "realty", "stocks", "education", "science", "society", "politics", "sports", "game",
                       "entertainment"]
    ...
    def test(self, config, texts):
        ...
        # 假设 outputs 是模型的输出
        if outputs.dim() > 1:
            # 如果 outputs 是二维的，按照原始代码获取最大值索引
            predic = torch.max(outputs.data, 1)[1].cpu().numpy()[0]
        else:
            # 如果 outputs 是一维的，直接获取最大值索引
            predic = outputs.argmax().item()  # 获取最大值的索引
        # predic = torch.max(outputs.data, 1)[1].cpu().numpy()[0]
        cls = self.labels[predic]
        return cls

    # python eval_FastText.py --model FastText --embedding random
